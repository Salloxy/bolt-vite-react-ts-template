...
import { Card, Rank, PokerHandRank, EvaluatedHand, Suit } from '../types/index.js';
import { getRankValue, RANKS } from './utils.js';
...
const getRankCounts = (cards: Card[]): Record<Rank, number> => {
...
const isFlush = (cards: Card[]): { flush: boolean; suit?: Suit } => {
...
const isStraight = (cards: Card[]): { straight: boolean; highRank?: Rank } => {
  ...
  if (isAceLow) return { straight: true, highRank: '5' }; // Ace-low straight, 5 is high card for ranking
...
export const evaluateHand = (hand: (Card | null)[]): EvaluatedHand | null => {
  const validCards = hand.filter(card => card !== null) as Card[];
  ...
  const ranksPresent = Object.keys(rankCounts) as Rank[];
...
export const compareEvaluatedHands = (handA: EvaluatedHand, handB: EvaluatedHand): number => {
  if (handA.rank > handB.rank) return 1; // Higher rank wins (PokerHandRank enum is ordered highest to lowest)
  ...
  for (let i = 0; i < handA.values.length; i++) {
    if (i >= handB.values.length) return 1; // handA has more kickers specified, should not happen if logic is correct
  ...
  if (handB.values.length > handA.values.length) return -1; // handB has more kickers
